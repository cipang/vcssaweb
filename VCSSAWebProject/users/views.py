...
EDITOR_GROUPNAME = {ADMIN_GROUPNAME[0]:"VCSSAEditor", ADMIN_GROUPNAME[1]:"UnimelbEditor", ADMIN_GROUPNAME[2]:"MonashEditor"}
...
def index(request):
    ...
    """Display all users to superuser, admin and editor from same union to admin"""
    print(users)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for group in usergroups:
                ...
                if group.name in ADMIN_GROUPNAME:
                    for item in Group.objects.filter(name=EDITOR_GROUPNAME.get(group.name)):
